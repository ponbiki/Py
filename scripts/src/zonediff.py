...
def lookup(record, type, ns):
    ...
    for answer in answers:
        if len(answer) != 0:
            component = ' '.join(answer.split("\t"))
            answers_list.append(component)
            sorted(answers_list)
        #sorted(results, key=itemgetter(len(results)-1))
        results.append(answers_list)
    return results
...
def diff_rec(list_o_records):
    ...
    for record in list_o_records:
        ...
        old_list.append(lookup(record['domain'], record['type'], legacy_ns))
        new_list.append(lookup(record['domain'], record['type'], NSONE_NS))
    diff_list = filter(lambda x:x not in new_list, old_list)
    return diff_list
...
fqdn = raw_input() #sanitize
legacy_ns = ns_get(fqdn)
list = record_list(curl_api(API_URI + "zones/" + fqdn, "GET", AUTH_HEAD + api_key))
for poop in list:
    pprint(lookup(poop['domain'], poop['type'], NSONE_NS))
